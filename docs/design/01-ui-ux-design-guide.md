...
5. **적응형 테마**: `--vscode-*` CSS 변수를 래핑하여 사용자의 커스텀 테마에 자동 적응

### Stitch 디자인 스크린

| 스크린 | 설명 | 다크 | 라이트 |
|--------|------|------|--------|
| Overview | 에이전트 카드 그리드 + 통계 + 도넛 차트 | ✅ | ✅ |
| Tasks (Kanban) | 3열 칸반 보드 (Pending/In Progress/Completed) | ✅ | ✅ |
| Messages | 스레드형 메시지 + 통계 패널 | ✅ | — |
| Activity | 세션 카드 + 필터 칩 + 타임라인 피드 | ✅ | — |
| Mini Dashboard | 상태 바 팝업 (4 메트릭) | ✅ | — |
| Timeline | 시간순 이벤트 타임라인 (날짜 그룹핑) | ✅ | — |
| Metrics | 도넛 차트 + 속도 차트 + 파일 히트맵 | ✅ | — |
| Sidebar Dashboard | 사이드바 미니 대시보드 (WebviewView) | ✅ | ✅ |
| SVG Icons | Stitch 브랜딩 기반 아이콘 세트 | ✅ | ✅ |

### 적응형 테마 전략 (3단계)

```
1단계: --vscode-* 변수 래핑 → 모든 사용자 테마에 자동 적응
       예: --cfm-bg-primary: var(--vscode-editor-background)
       → Dracula, One Dark Pro, Solarized 등 어떤 테마든 자동 상속

2단계: data-vscode-theme-kind 속성으로 4가지 모드 분기
       → vscode-dark, vscode-light, vscode-high-contrast, vscode-high-contrast-light
       → overlay, shadow, 상태 색상 대비 등 모드별 미세 조정

3단계: --cfm-* 시맨틱 토큰으로 도메인 색상 정의
       → 상태(completed/blocked), 에이전트(active/idle/lead) 등
       → RGB 분해 패턴으로 투명도 조절 용이
```

이 3단계 전략으로 VS Code 기본 테마는 물론, Dracula, One Dark Pro, GitHub Light,
Solarized, Nord 등 서드파티 커스텀 테마에서도 자연스럽게 작동한다.
...
### 2.3 다크/라이트 테마 색상 비교 (Stitch 기준)

| 토큰 | 다크 모드 | 라이트 모드 | 용도 |
|------|-----------|-------------|------|
| bg-primary | `#1e1e1e` | `#ffffff` | 에디터 배경 |
| bg-secondary | `#252526` | `#f3f3f3` | 사이드바 배경 |
| bg-tertiary | `#2d2d2d` | `#f5f5f5` | 위젯/카드 배경 |
| text-primary | `#cccccc` | `#333333` | 본문 텍스트 |
| text-secondary | `#999999` | `#616161` | 설명 텍스트 |
| text-muted | `#666666` | `#a0a0a0` | 비활성 텍스트 |
| border | `rgba(255,255,255,0.06)` | `#e0e0e0` | 테두리 |
| shadow-card | `0 2px 8px rgba(0,0,0,0.3)` | `0 1px 4px rgba(0,0,0,0.1)` | 카드 그림자 |
| status-completed | `#4caf50` | `#2e7d32` | 완료 (라이트에서 진하게) |
| status-in-progress | `#ff9800` | `#e65100` | 진행중 |
| status-blocked | `#f44336` | `#c62828` | 차단됨 |
| agent-active | `#2196f3` | `#1976d2` | 에이전트 활성 |
| agent-lead | `#ffd700` | `#b8860b` | 에이전트 리더 |

> **핵심**: 라이트 모드에서는 상태/에이전트 색상을 더 진하게(채도↑, 명도↓) 조정하여
> 밝은 배경 대비 WCAG AA 기준(4.5:1)을 충족한다.
